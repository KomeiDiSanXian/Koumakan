...
func init() {
	...
	if file.IsExist(lnfile) {
		...
		if err != nil {
			...
			mun, err := strconv.Atoi(helper.BytesToString(data))
			if err != nil {
	...
	OnCommandGroup([]string{
		"响应", "response", "沉默", "silence",
	}, UserOrGrpAdmin, OnlyToMe).SetBlock(true).SetPriority(1).Handle(func(ctx *Ctx) {
		...
		case "响应", "response":
			...
			if err == nil {
				msg = message.Text(BotConfig.NickName[0], "将开始在此工作啦~")
			} else {
		...
		case "沉默", "silence":
			...
			if err == nil {
				msg = message.Text(BotConfig.NickName[0], "将开始休息啦~")
			} else {
				msg = message.Text("ERROR: ", err)
			}
		default:
	...
	OnCommandGroup([]string{
	...
	}, SuperUserPermission, OnlyToMe).SetBlock(true).SetPriority(1).Handle(func(ctx *Ctx) {
		var msg message.MessageSegment
		...
		case strings.Contains(cmd, "响应") || strings.Contains(cmd, "response"):
			...
			if err == nil {
				msg = message.Text(BotConfig.NickName[0], "将开始在全部位置工作啦~")
			} else {
				msg = message.Text("ERROR: ", err)
			}
		case strings.Contains(cmd, "沉默") || strings.Contains(cmd, "silence"):
			...
			if err == nil {
				msg = message.Text(BotConfig.NickName[0], "将开始在未显式启用的位置休息啦~")
			} else {
				msg = message.Text("ERROR: ", err)
			}
		default:
	...
	OnCommandGroup([]string{
		...
		if condition {
			managers.ForEach(func(key string, manager *control.Control[*Ctx]) bool {
				if manager.Options.DefaultDisable == condition {
					return true
				}
				manager.Enable(grp)
				return true
	...
	}, OnlyToMe, SuperUserPermission).SetBlock(true).SetPriority(1).Handle(func(ctx *Ctx) {
		model := extension.CommandModel{}
		_ = ctx.Parse(&model)
		service, ok := Lookup(model.Args)
		if !ok {
	...
	OnCommandGroup([]string{"还原", "reset"}, UserOrGrpAdmin, OnlyToMe).SetBlock(true).SetPriority(1).Handle(func(ctx *Ctx) {
		model := extension.CommandModel{}
		_ = ctx.Parse(&model)
		service, ok := Lookup(model.Args)
		if !ok {
	...
	OnCommandGroup([]string{
		"禁止", "ban", "允许", "permit",
	}, AdminPermission, func(ctx *Ctx) bool {
		model := extension.CommandModel{}
		_ = ctx.Parse(&model)
		...
		var uid int64
		var err error
		haspermission := GroupHigherPermission(func(ctx *Ctx) int64 { return uid })
		...
		return true
	}, OnlyToMe).SetBlock(true).SetPriority(1).Handle(func(ctx *Ctx) {
	...
	OnCommandGroup([]string{"用法", "usage"}).SetBlock(true).SetPriority(1).
		Handle(func(ctx *Ctx) {
			model := extension.CommandModel{}
			_ = ctx.Parse(&model)
			service, ok := Lookup(model.Args)
			...
			imgs, err := (&rendercard.Title{
				...
				IsEnabled:     service.IsEnable(gid),
	...
	OnCommandGroup([]string{"服务列表", "service_list"}).SetBlock(true).SetPriority(1).
		Handle(func(ctx *Ctx) {
			gid := ctx.Event.GroupID
			if gid == 0 {
				gid = -ctx.Event.UserID
			...
			if len(imgs) > 1 {
				...
				for i := 0; i < len(imgs); i++ {
					go func(i int) {
						defer wg.Done()
						msg[i] = FakeSenderForwardNode(ctx, message.Image(helper.BytesToString(binary.NewWriterF(func(w *binary.Writer) {
				...
				if id := ctx.SendChain(message.Image("base64://" + helper.BytesToString(b64))); id.ID() == 0 {
					ctx.SendChain(message.Text("ERROR: 可能被风控了"))
				}
			}
		})

	OnCommand("设置服务列表显示行数", SuperUserPermission, OnlyToMe).SetBlock(true).SetPriority(1).Handle(func(ctx *Ctx) {
		model := extension.CommandModel{}
		_ = ctx.Parse(&model)
		...
		err = os.WriteFile(lnfile, helper.StringToBytes(model.Args), 0644)
		if err != nil {
